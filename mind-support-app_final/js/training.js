...
const Training = {
  ...
  currentStep: 0,
  attempts: 0,
  viewLevel: null, // 手动查看等级

  // ---- 根据等级获取训练场景 ----
  getTrainingScenesByLevel() {
    const level = this.viewLevel || App.getUserLevel().id;
  ...
  renderTrainingList() {
    ...
    container.innerHTML = `
      ...
      <div style="display:flex; gap:8px; justify-content:center; margin-bottom:20px; flex-wrap:wrap;">
        ...
        <button class="btn btn-outline" onclick="Training.switchViewLevel('low')" style="padding:6px 16px; font-size:0.9rem; ${level === 'low' ? 'background:var(--level-low);color:white;border-color:var(--level-low);' : ''}">🌱 基础级</button>
        <button class="btn btn-outline" onclick="Training.switchViewLevel('medium')" style="padding:6px 16px; font-size:0.9rem; ${level === 'medium' ? 'background:var(--level-mid);color:#D48806;border-color:var(--level-mid);' : ''}">⭐ 中级</button>
        <button class="btn btn-outline" onclick="Training.switchViewLevel('high')" style="padding:6px 16px; font-size:0.9rem; ${level === 'high' ? 'background:var(--level-high);color:#389E0D;border-color:var(--level-high);' : ''}">🏆 高级</button>
        ${isManual ? `<button class="btn btn-outline" onclick="Training.resetViewLevel()" style="padding:6px 16px; font-size:0.9rem;">🔄 跟随评估</button>` : ''}
      </div>
    ` + Object.entries(scenes).map(([key, scene]) => {
      // Get training status
      const trainingStatus = App.store.get('training_completed', {});
      const sceneStatus = trainingStatus[key];
      ...
      <div class="training-card" onclick="Training.startScene('${key}')">
        ...
        ${sceneStatus ? `<div style="display:flex; align-items:center; gap:6px; margin-top:8px;">
          <span style="background:var(--success); color:white; padding:4px 12px; border-radius:12px; font-size:0.85rem; font-weight:600;">✅ 已完成训练</span>
          <span style="font-size:0.8rem; color:var(--text-light);">${sceneStatus.completedAt}</span>
        </div>` : `<div style="margin-top:8px;">
          <span style="background:var(--bg-warm); color:var(--text-secondary); padding:4px 12px; border-radius:12px; font-size:0.85rem;">⏳ 未训练</span>
        </div>`}
      </div>`;
    }).join('');
  },
  ...
  updateTrainingStep() {
    ...
    // 渲染步骤动画
    const animArea = document.getElementById('trainingAnimArea');
    if (animArea && typeof StepAnimation !== 'undefined') {
      animArea.innerHTML = StepAnimation.getAnimation(step.title, step.hint);
    } else if (animArea) {
      animArea.innerHTML = '';
    }
  ...
  completeTraining() {
    ...
    const trainingStatus = App.store.get('training_completed', {});
    trainingStatus[this.activeScene] = {
      completed: true,
      sceneName: scene.name,
      steps: scene.steps.length,
      attempts: this.attempts,
      completedAt: new Date().toLocaleString('zh-CN'),
      level: this.viewLevel || App.getUserLevel().id
    };
    App.store.set('training_completed', trainingStatus);
  ...
  // Check if any training has been completed
  hasAnyCompleted() {
    const trainingStatus = App.store.get('training_completed', {});
    return Object.keys(trainingStatus).length > 0;
  },

  // Get completed training count
  getCompletedCount() {
    const trainingStatus = App.store.get('training_completed', {});
    return Object.keys(trainingStatus).length;
  }
};
